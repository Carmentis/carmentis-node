...
Target: abci/ (NestJS ABCI++ application for CometBFT v0.38), commit a701f1b Scope: Full application: ABCI handlers, state machine, account/token logic, database & persistence layer, snapshots/state-sync, genesis, validator-set updates, AppHash computation. Method: Every ABCI++ handler traced CheckTx→Commit; state machine reconstructed; AppHash inputs traced; determinism, atomicity, monetary conservation and authorization examined; counterexamples constructed. The application relies heavily on @cmts-dev/carmentis-sdk-core (microblock parsing, BalanceAvailability, CMTSToken, consistency checking); SDK behavior is flagged where load-bearing.
1. Executive Summary
...
1.
CRITICAL — Expired-escrow daily sweep halts the chain when a single account is the payee of ≥2 escrows expiring on the same day. Deterministic across all validators → permanent halt at that day boundary. (§3.1)
2.
CRITICAL — ProcessProposal accepts every proposal unconditionally, while FinalizeBlock silently drops invalid txs from tx_results. A proposer including one invalid microblock makes len(tx_results) != len(txs); CometBFT v0.38 turns this into an ApplyBlock error → consensus panic on every node → halt. (§3.2)
3.
HIGH — Non-atomic commit + swallowed storage-write errors + non-idempotent block replay create several crash-recovery paths to a single-node (or, at a day boundary, all-node) halt or stuck node. (§3.3, §6)
4.
HIGH — PrepareProposal/ProcessProposal mutate persistent state (indexVirtualBlockchain writes directly to LevelDB), violating ABCI++ and corrupting the query index with non-committed objects. Does not affect AppHash. (§5, ABCI Compliance)
...
5. Critical Findings (summary table)
   ...
   Class
   C-1
   Expired-escrow daily sweep double-deletes ESCROWS → throw → halt
   CRITICAL
   GlobalStateUpdater.ts:136-155 + AccountEscrowHandler.ts:78-119
   Chain halt (liveness), unprivileged trigger
   C-2
   ProcessProposal accepts all + FinalizeBlock tx_results count mismatch → consensus panic
   CRITICAL
   AbciService.ts:839-894, :904-992
   Chain halt + ABCI++ violation
6. High Findings
   ID
   Title
   Location
   Notes
   H-1
   PrepareProposal/ProcessProposal mutate persistent state. GlobalState.indexVirtualBlockchain (state/GlobalState.ts:301-309) writes via this.db.indexVirtualBlockchain (database/LevelDb.ts:274, immediate putRaw, not buffered) and is called from updateGlobalStateOnMicroblock (GlobalStateUpdater.ts:451) during proposal phases. Index entries for VBs in rejected/uncommitted proposals persist permanently.
   state/GlobalState.ts:301, GlobalStateUpdater.ts:249,451
   ABCI++ violation. Does not affect AppHash (indexes excluded from radix), but corrupts the queryable object index (getObjectList) and is non-atomic vs. Commit. The same call also bypasses the atomic batch in FinalizeBlock. Fix: route index writes through the cached/buffered DB and commit them atomically in Commit.
   H-2
   Misbehavior slashing halt (no staking lock)
   GlobalStateUpdater.ts:84-104, AccountSlashingHandler.ts:12
   §3.3
   H-3
   Unknown-validator fee-dispatch halt
   GlobalStateUpdater.ts:680
   §3.4
   H-4
   Non-atomic commit + swallowed storage errors. GlobalState.commit (state/GlobalState.ts:245-248) commits the DB batch then flushes storage; Storage.writeTransactions (storage/Storage.ts:209-211) catches and swallows all errors (incl. the size-mismatch "PANIC"). A failed/partial flush leaves DB ahead of disk; the next storage challenge's checkSizeOrFail (challenge/ChallengeFile.ts:33) throws in FinalizeBlock → node halt.
   state/GlobalState.ts:245, storage/Storage.ts:175-212
   Persistence/recovery. Fix: order storage-before-DB (or use a write-ahead marker), and never swallow write errors — fail loudly and abort Commit.
   H-5
   Non-idempotent block replay. updateGlobalStateOnBlock throws duplicate block at height N (GlobalStateUpdater.ts:69-75) if getBlockContent(N) exists. If CometBFT replays the last committed block after a crash between DB commit and CometBFT recording the commit, FinalizeBlock(N) throws → node cannot restart.
   GlobalStateUpdater.ts:61-76
   Crash recovery. Fix: make replay of the last block idempotent (detect already-applied height and return the stored result/AppHash).
   H-6
   Snapshot import is non-atomic. SnapshotsManager.importDbFile calls db.clear() then repopulates in batches; a crash mid-import leaves a truncated DB and a garbage AppHash.
   SnapshotsManager.ts (importDbFile)
   Fix: stage into a temp DB and atomically swap after a full hash-verified import.
   H-7
   Snapshot/state-sync verification gap. OfferSnapshot ACCEPTs unconditionally and ApplySnapshotChunk never verifies chunks against the advertised snapshot hash. CometBFT's post-restore AppHash-vs-header check mitigates state injection for normal state-sync, but the genesis bootstrap path (initChainAsNode → searchGenesisSnapshotChunksFromGenesisNode, AbciService.ts:332-404) imports an RPC-fetched snapshot with no integrity check and returns a self-derived AppHash from InitChain (no trusted header to compare against unless genesis.json pins app_hash).
   AbciService.ts:1089-1127, :332-404, SnapshotsManager.loadReceivedChunk
   Fix: recompute and compare snapshot hash on apply; anchor the genesis snapshot hash in the genesis doc and verify before importing; reject on mismatch.
   H-8
   Snapshot-apply DoS (unguarded throws on malformed chunks)
   SnapshotDataCopyManager, SnapshotsManager.importDbFile, GrpcAbciController (no try/catch)
   §3.6
7. Medium Findings
   •
...
9. ABCI++ Compliance Assessment
   ...
   Info
   OK
   Returns last_block_height + last_block_app_hash from committed state. Sets isCatchingUp=false (used only to gate local snapshot creation).
   InitChain
   OK w/ risks
   Genesis path builds state; non-genesis path state-syncs via RPC network call (MED-10) with no integrity check (H-7). Clears DB/storage/snapshots first.
   CheckTx
   OK
   Parses microblock, early-reject cache, min gas price, signature & consistency checks. Uses wall-clock referenceTimestamp — acceptable (mempool admission, not consensus). Fully wrapped in try/catch; invalid txs rejected, never crash.
   PrepareProposal
   VIOLATION
   Mutates persistent state via indexVirtualBlockchain (H-1). Can throw out of the handler at a day boundary (C-1). Gas-descending sort is proposer-local (fine).
   ProcessProposal
   VIOLATION
   Always returns ACCEPT — performs no validation/rejection (C-2 root cause). Also mutates state via indexVirtualBlockchain (H-1).
   FinalizeBlock
   VIOLATION
   tx_results count can ≠ txs count (C-2). Day-boundary sweep can throw (C-1, H-2, H-3). Otherwise performs the real state transition correctly.
   Commit
   Partial
   DB batch is atomic, but storage flush is separate, after the DB and swallows errors (H-4); also drives local snapshot creation/pruning.
   ExtendVote
   OK*
   No-op; correct only while vote extensions are disabled (LOW-3).
   VerifyVoteExtension
   OK*
   Accepts unconditionally; safe only while disabled (LOW-3).
...
1.
C-1: In updateEscrowLocks, if (escrow == null) continue; and/or make updateExpiredEscrows process only the iterated escrow id. Make all day-boundary sweeps non-throwing.
2.
C-2: Always append one tx_result per tx in FinalizeBlock (non-zero code for rejected); make ProcessProposal reject blocks containing microblocks that FinalizeBlock would not apply. Keep the two validators identical.
3.
H-4 / H-5 / H-6: Make persistence crash-safe: storage-before-DB (or WAL marker), never swallow storage write errors, make last-block replay idempotent, and make snapshot import atomic (stage + swap).
4.
H-1: Buffer index writes through CachedLevelDb and commit them atomically in Commit; never write persistent state during Prepare/Process.
5.
H-2 / H-3 / 3.5: Replace consensus-path throws (slashing without lock, unknown voter, protocol-index cardinality) with log-and-skip / guaranteed invariants.
6.
H-7 / H-8 / MED-10: Verify snapshot/genesis-snapshot content against an anchored hash; wrap snapshot handlers; reject (don't crash) on malformed input; pin the genesis snapshot hash in genesis.json.
7.
MED-1 / MED-2 / MED-3: Canonicalize getKeys ordering and CBOR encoding; constrain numeric schemas to integers; remove the undefined→0 timestamp fallback (throw / enforce monotonicity).
8.
MED-4: Migrate atomic amounts and voting power to bigint with isSafeInteger assertions.
9.
MED-5 / MED-6 / MED-9: Clean up staking-lock index after slashing; fix the block_id_flag comparison; use block timestamp for unstake/escrow timing.
10.
Address LOW items (vote-extension no-ops if ever enabling extensions, dead code, debug logging, key-length checks).
