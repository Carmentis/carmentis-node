...
export class RandomNumberService {
    ...
    async sendRandomNumber(): Promise<void> {
        ...
        try {
            ...
            this.logger.log(
                `Sent random number: ${randomNumber}, Status: ${response.status}, Hash: ${response.data['result']['hash']}`,
            );

            setTimeout(async () => {
                const tx = await firstValueFrom(
                    this.httpService.post<object>(urlTx + response.data['result']['hash']).pipe(
                        catchError((error) => {
                            throw error;
                        }),
                    ),
